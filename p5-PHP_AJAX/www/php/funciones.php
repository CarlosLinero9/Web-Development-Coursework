...
function obtener_noticia(mysqli $conexion, int $id): ?array
{
    $sql = "
        SELECT n.id, n.titulo, n.fecha_publicacion, n.tipo,
               ...
               l.nombre AS lugar, n.descripcion, n.publicado
        FROM noticias n
        INNER JOIN lugares l ON l.id = n.lugar_id
        WHERE n.id = ?
    ";
...
{
    $sql = "
        SELECT n.id, n.titulo, n.fecha_publicacion, n.tipo, n.publicado, l.nombre AS lugar
        FROM noticias n
        INNER JOIN lugares l ON l.id = n.lugar_id
        WHERE 1 = 1
    ";
...
function buscar_noticias_portada_ajax(mysqli $conexion, string $busqueda): array
{
    $like = '%' . $busqueda . '%';
    $stmt = $conexion->prepare('SELECT id, titulo FROM noticias WHERE publicado = 1 AND titulo LIKE ? ORDER BY fecha_publicacion DESC, id DESC LIMIT 8');
...
function buscar_noticias_gestion_ajax(mysqli $conexion, string $busqueda): array
{
    if ($busqueda === '') {
        $stmt = $conexion->prepare('SELECT n.id, n.titulo, n.fecha_publicacion, n.tipo, n.publicado, l.nombre AS lugar FROM noticias n INNER JOIN lugares l ON l.id = n.lugar_id ORDER BY n.fecha_publicacion DESC, n.id DESC');
    } else {
        $like = '%' . $busqueda . '%';
        $stmt = $conexion->prepare('SELECT n.id, n.titulo, n.fecha_publicacion, n.tipo, n.publicado, l.nombre AS lugar FROM noticias n INNER JOIN lugares l ON l.id = n.lugar_id WHERE n.titulo LIKE ? OR n.descripcion LIKE ? ORDER BY n.fecha_publicacion DESC, n.id DESC');
...
function cambiar_estado_publicado(mysqli $conexion, int $id, int $publicado): bool
{
    $stmt = $conexion->prepare('UPDATE noticias SET publicado = ? WHERE id = ?');
    $stmt->bind_param('ii', $publicado, $id);
    $stmt->execute();
...
{
    $publicado = isset($datos['publicado']) ? (int) $datos['publicado'] : 1;
    $stmt = $conexion->prepare('INSERT INTO noticias (titulo, fecha_publicacion, tipo, concejalia, personas_responsables, lugar_id, descripcion, publicado) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    $stmt->bind_param('sssssisi', $datos['titulo'], $datos['fecha_publicacion'], $datos['tipo'], $datos['concejalia'], $datos['personas_responsables'], $datos['lugar_id'], $datos['descripcion'], $publicado);
    $stmt->execute();
    $id = $conexion->insert_id;
    $stmt->close();
    return (int) $id;
...
{
    $publicado = isset($datos['publicado']) ? (int) $datos['publicado'] : 1;
    $stmt = $conexion->prepare('UPDATE noticias SET titulo = ?, fecha_publicacion = ?, tipo = ?, concejalia = ?, personas_responsables = ?, lugar_id = ?, descripcion = ?, publicado = ? WHERE id = ?');
    $stmt->bind_param('sssssisii', $datos['titulo'], $datos['fecha_publicacion'], $datos['tipo'], $datos['concejalia'], $datos['personas_responsables'], $datos['lugar_id'], $datos['descripcion'], $publicado, $id);
    $stmt->execute();
    $stmt->close();
}
